...
```{r plot horses, fig.width=10, fig.height=3, fig.cap = "Figure 1. Horse population for all provinces in Canada between 1940 and 1972.", out.width = "100%"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
max_horses |> kable(
  caption = "Table 1. Maximum and minimum number of horses in each province between 1940 and 1972"
)
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=5, fig.height=4, fig.cap=paste("Figure 2. Number of horses for", params$province, "between 1940 and 1972"), out.width= "50%"}
